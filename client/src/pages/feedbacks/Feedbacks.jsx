...
export default function Feedbacks() {
    ...
    const fetchFeedbacks = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/users/${id}/feedbacks`,
          {
  ...
  return (
    <div class="feedbacks">
      <h3 className="profile-page__header">Feedbacks</h3>
      <div className="rating">
        {/* <span className="rating__count">(100 feedbacks) </span> */}
      ...
      <div className="review-category-container">
        ...
        <button className="review-category-item btn btn-1">5 stars ({feedbacks.filter((f) => f.rating === 5).length})</button>
        <button className="review-category-item btn btn-1">4 stars ({feedbacks.filter((f) => f.rating === 4).length})</button>
        <button className="review-category-item btn btn-1">3 stars ({feedbacks.filter((f) => f.rating === 3).length})</button>
        <button className="review-category-item btn btn-1">2 stars ({feedbacks.filter((f) => f.rating === 2).length})</button>
        <button className="review-category-item btn btn-1">1 star ({feedbacks.filter((f) => f.rating === 1).length})</button>
      </div>
            ...
            <div className="review-item">
              <div className="review-item--left">
                <div className="user-info">
                  <img src={review.avatar} alt="" className="user-info__avt" />
