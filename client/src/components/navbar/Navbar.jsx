...
import DefaultAvt from "../../assets/img/default_avt.png";
import Menu from "../menu/Menu.jsx";
import Cart from "../cart/Cart.jsx";
import ShowcasingArtworkDetails from "../../pages/showcasingArtworkDetails/ShowcasingArtworkDetails.jsx";
import "./Navbar.scss";
...
const Navbar = () => {
  ...
  useEffect(() => {
    // Check if talents or artworks have items and update showDropdown accordingly
    if (
      (searchOptions.talents && searchOptions.talents.length > 0) ||
      (searchOptions.artworks && searchOptions.artworks.length > 0)
    ) {
  ...
  const handleSearchChange = async (e) => {
    ...
    if (query.length > 0) {
      ...
      const response = await axios.get(
        `http://localhost:3000/common/search?query=${query}`
      );
      ...
      setQuery("");
      setIsFocused(false);
      setSearchOptions({ talents: [], artworks: [] }); // Reset search options when query is empty
    }
  };
    ...
    <nav className="nav">
      <div className="nav-left">
        ...
        <div
          ...
          onBlur={blurInput}
        >
          <form className="form-field" onSubmit={handleSearchSubmit}>
            <img
            ...
            <div className="dropdown-search-container">
              {searchOptions?.talents && searchOptions.talents.length > 0 && (
                <>
                  ...
                  <hr />
                  <ul>
                    {searchOptions.artworks.map((artwork) => 
               ...
               {searchOptions?.talents && searchOptions.talents.length > 0 && (
                <>
                  ...
                  <hr />
                  <ul>
                    {searchOptions.talents.map((talent) => 
                      <li
                        key={talent._id}
                        className="dropdown-search-item"
                      >
                        <img src={talent.avatar ? talent.avatar : "https://cdn.esawebb.org/archives/images/screen/carinanebula3.jpg"} alt="Artist Avatar" />
              ...
              {(searchOptions.artworks.length === 0) && 
              (searchOptions.talents.length === 0) &&
              (<p className="extra-msg gray">No results found</p>)}
            </div>
          )}
        </div>
      </div>
