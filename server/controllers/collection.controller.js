...
class CollectionController {
  ...
  index = async (req, res, next) => {
    ...
    try {
      collections = await redisHandling.getFromRedis(
        `collections/talentId:${q.talentId}`
      );
      if (!collections) {
  ...
  store = async (req, res, next) => {
    ...
    try {
      ...
      let collectionsInRedis = await redisHandling.getFromRedis(listKey);
      if (Array.isArray(collectionsInRedis)) {
        collectionsInRedis = collectionsInRedis.push(savedCollection);
        await redisHandling.setToRedis(listKey, collectionsInRedis);
      }
  ...
  show = async (req, res, next) => {
    ...
    let collection;
    try {
      collection = await redisHandling.getFromRedis(
  ...
  update = async (req, res, next) => {
    ...
    try {
      ...
      //Redis
      const collectionInRedis = await redisHandling.getFromRedis(
        `collections/${collectionId}`
      );
      ...
      let collectionsInRedis = await redisHandling.getFromRedis(listKey);
      if (Array.isArray(collectionsInRedis)) {
        collectionsInRedis = collectionsInRedis.map((collection) => {
  ...
  destroy = async (req, res, next) => {
    ...
    try {
      ...
      let collectionsInRedis = await redisHandling.getFromRedis(listKey);
      if (Array.isArray(collectionsInRedis)) {
        collectionsInRedis = collectionsInRedis.filter(
